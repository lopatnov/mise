...
- **web** — `npm ci` → `lint` → `check:locales` → `build` → `test:a11y` (WCAG 2.1 A/AA
  ...
  билда — см. Testing выше; падает CI при найденных нарушениях). `color-contrast` временно
  занесён в `ignore` в `web/.pa11yci.json` — известная реальная проблема (`--c-primary`
  недостаточно контрастен как цвет мелкого текста на светлом фоне), трекается в issue #75,
  не притворяется отсутствующей.
