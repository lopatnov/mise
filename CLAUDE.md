...
не обязательно выносить хук (в отличие от pressmark, здесь это не единственный принятый паттерн).
...
- **Весь код, комментарии, коммиты, issues и PR — на английском** (проект — публичный OSS на
  английском, `.claude/`-файлы процесса — на русском, языке рабочего диалога с пользователем).
...
- `CONTRIBUTING.md` ссылается на этот файл (`./CLAUDE.md`) как на источник бэклога — актуальный
  бэклог живёт в GitHub Issues, этот файл описывает процесс и стек, не список задач.
