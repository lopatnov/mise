...
export const adminApi = {
  setupStatus: () =>
    api.get<{ setupDone: boolean }>('/admin/setup').then((r) => r.data),
  setup: (email: string, password: string, displayName?: string) =>
    api.post('/admin/setup', { email, password, displayName }).then((r) => r.data),

  getSettings: () =>
    api.get<AppSettings>('/admin/settings').then((r) => r.data),
  updateSettings: (data: Partial<AppSettings>) =>
    api.patch<AppSettings>('/admin/settings', data).then((r) => r.data),
