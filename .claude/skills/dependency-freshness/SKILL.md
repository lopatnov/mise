...
3. Для MAJOR-апдейтов — `npm install <pkg>@latest` (для уже объявленной зависимости `npm install
   ...
   **не полагайся молча** на то, что диапазон, который npm сам запишет в `package.json`, совпадает
   ...
   `package.json` (`api/`, `web/`) в подавляющем большинстве используют `^`-диапазоны — значит после
   `@latest` в `package.json` обычно должен остаться `^<новая версия>`. Исключения уже есть и это
   нормально, не баг: `web/package.json` пинует `typescript` через `~` (минорные TS-версии бывают
   breaking), `api/package.json` — `picomatch` без префикса в `overrides` (транзитивная зависимость,
   не под тем же контролем, что и обычная). Если апдейтишь именно такой пакет — сохрани его
   существующий стиль пиновки, не переводи молча на `^`; для всех остальных сверь, что npm не
   записал что-то отличное от принятого в проекте `^`. Для minor/patch
